...
def get(url, *args, html=None, **request_kwargs):
    """Get all fav icons for a url.

    :param url: Homepage.
    :type url: str

    :param html: HTML input, as string. Will be used instead of an HTTP response from the `url`.
    ...
    link_icons = tags(final_url, html)
    if link_icons:
